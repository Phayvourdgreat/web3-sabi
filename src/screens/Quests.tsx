...
function CreateWalletQuest({
  ...
  return (
    <div className="pb-24 animate-fade-in">
      <Header title="Quest 1: Create a wallet" onBack={onBack} />
...
export default function Quests({ onBack }: QuestsProps) {
  ...
  return (
    <div className="pb-24 animate-fade-in">
      <Header title="Learn by doing" onBack={onBack} />

      <div className="px-5 space-y-4">
        <p className="text-sm text-gray-400">
          Finish real Web3 quests on a test network. No real money is used.
        </p>
        ...
        <div className="space-y-2.5">
          {QUESTS.map((quest, index) => {
            ...
            return (
              <button
                key={quest.key}
                onClick={() => !locked && setActive(quest.key)}
                disabled={locked}
